...
def initialize_tp_communicators(
    ...
    assert (tokens is not None and tokens > 0) ^ (
        seq_length is not None and seq_length > 0 and micro_batch_size is not None and micro_batch_size > 0
    ...
    if seq_length is not None:
        input_shape = [
            (seq_length * micro_batch_size * mpu.get_tensor_model_parallel_world_size()) // mpu.get_context_parallel_world_size(),
            hidden_size,
        ]
    else:
        input_shape = [tokens * mpu.get_tensor_model_parallel_world_size() // mpu.get_context_parallel_world_size(), hidden_size]
