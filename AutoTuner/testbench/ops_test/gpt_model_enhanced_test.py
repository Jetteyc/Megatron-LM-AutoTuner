...
from AutoTuner.testbench.ops.gpt_model_enhanced import GPTModelEnhancedForTest
...
class TestGPTModelEnhanced(TestCommon):
    def __init__(
        self,
        ...
        **kwargs,
        ...
        self.module_name = "GPTModelEnhanced"
        ...
        if profile_mode == ProfileMode.collect_data:
            with MemoryTrackerContext(self.module_name) as memory_tracker_ctx:
                self.op = GPTModelEnhancedForTest(
                    tf_config=tf_config,
                    hf_config=hf_config,
    ...
    def _calculate_weight_memory(
        ...
        if pp_rank == 0:
            ...
            # Position embeddings
            estimated_weight_bytes += (
                hf_config.max_position_embeddings * hidden_size * bytes_per_param
            )
            # Tokentype embeddings
            if (
                hasattr(self.op.embedding, "tokentype_embeddings")
                and self.op.embedding.tokentype_embeddings is not None
            ):
