...
def plot_memory_compare_all_eps(
    ...
    plt.yticks(fontsize=tick_fontsize)
    plt.xlabel("专家并行机器数量", fontsize=axis_label_fontsize)
    plt.ylabel("GPU 最大显存（GiB）", fontsize=axis_label_fontsize)
    plt.title("不同专家并行机器数量下的 GPU 最大显存对比", fontsize=title_fontsize)
...
def plot_summary_curves(
    ...
    plt.title("不同专家并行机器数量大小下的 Token 方差对比（log10）", fontsize=title_fontsize)
