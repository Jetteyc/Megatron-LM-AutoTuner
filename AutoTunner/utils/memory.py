...
class ActivationHook:
    def __init__(self, enable=True, module_name="", logging_level=logging.INFO):
    ...
    def save_hook(self, x):
        if self.enable:
            log_rank0(f"[{self.module_name} save] tensor shape={tuple(x.shape)}, dtype={x.dtype}, device={x.device}", level=self.logging_level)
            self.activation_tensors.append(x)
        return x  # 必须返回 x，否则计算图会出错

    def load_hook(self, x):
        if self.enable:
            log_rank0(f"[{self.module_name} load] tensor shape={tuple(x.shape)}, dtype={x.dtype}, device={x.device}", level=self.logging_level)
        return x
