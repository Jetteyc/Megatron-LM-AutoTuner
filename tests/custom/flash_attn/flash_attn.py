...
def benchmark_flash_attention(batch_sizes, seq_lens, num_heads=8, head_dim=64, dropout=0.0, causal=True, dtype=torch.float16, device='cuda', warmup=5, iterations=20):
    results = []

    print(f"{'Batch':<8} {'SeqLen':<8} {'Forward (ms)':<15} {'Backward (ms)':<15} {'Total (ms)':<12} {'Memory (GB)':<12}")
    print("-" * 75)
    ...
    for bsz in batch_sizes:
        for seqlen in seq_lens:
            ...
            print(f"bsz: {bsz:<8} seqlen: {seqlen:<8} fwd_time: {fwd_time:<15.3f} bwd_time: {bwd_time:<15.3f} total_time: {total_time:<12.3f} memory: {memory:<12.2f}")
...
def plot_results(args, results):
    ...
    plt.tight_layout()
    plt.savefig(f"{args.output_dir}/flash_attention_benchmark.png", dpi=150, bbox_inches='tight')
...
def main():
    ...
    parser.add_argument('--seq-lens', nargs='+', type=int, default=[512, 1024, 2048, 4096, 8192],
    ...
    parser.add_argument('--no-causal', action='store_false', dest='causal', help='Disable causal mask (use bidirectional)')
    ...
    parser.add_argument('--output-dir', type=str, default="outputs/test/custom/flash_attn", help="Output directory")
    parser.add_argument('--draw', action='store_true', help='Plot the benchmark results')
