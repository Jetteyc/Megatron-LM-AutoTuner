...
TORCH_PROFILER_OUTPUT_DIR = "outputs/functional/cpu_gpu_movements/interference_with_comm/torch_profiler"
...
def init_distributed(rank: int, world_size: int, backend: str = "nccl"):
    ...
    torchcomm = new_comm("ncclx", device, name="main_comm", timeout=datetime.timedelta(seconds=300))
    print(f"Initialized process {rank} of {world_size} on backend {backend}, torchcomm: {torchcomm.get_rank()}")
    return torchcomm
