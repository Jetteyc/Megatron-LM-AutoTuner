...
class TestLanguageModelEmbedding:
    def __init__(
        ...
        self.memory_db["weights"].update({self.module_name: memory_tracker_ctx.get_memory_usage()})
        self.test_cases = test_cases
        self.model_config = hf_config
        self.profile_mode = profile_mode
        self.warmup = warmup
    
    def test_one_case(self, batch_size, seqlen, shape, system):
        input_ids, attention_mask, position_ids, packed_sequence_params  = get_model_input(
            self.model_config, batch_size, seqlen, shape, system
        )
        ...
        self.memory_db["activations"].update({name: self.embedding.get_activation_memory()})
